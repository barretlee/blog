  ...
  var t = function() {
    function t(e) {
      ...
      this.playIndex = "[object Array]" === Object.prototype.toString.call(e.music) ? 0 : -1, this.option = e, this.audios = [], this.loop = e.loop, this.secondToTime = function(e) {
        ...
        for (var t = [], a = 0; a < e.length; a++) {
          for (var i = e[a].split("\n"), s = [], n = i.length, l = 0; n > l; l++) {
            var o = i[l].match(/\[(\d{2}):(\d{2})\.(\d{2,3})]/g),
              r = i[l].replace(/\[(\d{2}):(\d{2})\.(\d{2,3})]/g, "").replace(/^\s+|\s+$/g, "");
            if (null != o)
              for (var d = o.length, u = 0; d > u; u++) {
                var p = /\[(\d{2}):(\d{2})\.(\d{2,3})]/.exec(o[u]),
        ...
        t = t > 0 ? t : 0, t = 1 > t ? t : 1, a[e + "Bar"].style[i] = 100 * t + "%"
    ...
    return _createClass(t, [{
      ...
      value: function() {
        ...
        if (this.option.showlrc) {
          ...
          if (1 === this.option.showlrc)
            if (this.multiple)
              for (s = 0; s < this.option.music.length; s++) n.push(this.option.music[s].lrc);
            else n.push(this.option.music.lrc);
          else if (2 === this.option.showlrc || this.option.showlrc === !0)
            for (s = 0; s < this.element.getElementsByClassName("aplayer-lrc-content").length; s++) n.push(this.element.getElementsByClassName("aplayer-lrc-content")[s].innerHTML);
        ...
        var r = function(e) {
            ...
            s = s > 0 ? s : 0, s = 1 > s ? s : 1, i.updateBar("played", s, "width"), i.option.showlrc && i.updateLrc(parseFloat(i.playedBar.style.width) / 100 * i.audio.duration), i.element.getElementsByClassName("aplayer-ptime")[0].innerHTML = i.secondToTime(s * i.audio.duration)
          },
          d = function h() {
            document.removeEventListener("mouseup", h), document.removeEventListener("mousemove", r), i.audio.currentTime = parseFloat(i.playedBar.style.width) / 100 * i.audio.duration, i.playedTime = setInterval(function() {
        ...
        this.thumb.addEventListener("mousedown", function() {
          o = i.bar.clientWidth, clearInterval(i.playedTime), document.addEventListener("mousemove", r), document.addEventListener("mouseup", d)
        }), this.volumeBar = this.element.getElementsByClassName("aplayer-volume")[0];
        var u = this.element.getElementsByClassName("aplayer-volume-bar")[0],
        ...
        this.element.getElementsByClassName("aplayer-volume-bar-wrap")[0].addEventListener("click", function(e) {
          var t = e || window.event,
            s = (c - t.clientY + a(u)) / c;
          s = s > 0 ? s : 0, s = 1 > s ? s : 1, i.updateBar("volume", s, "height"), i.audio.volume = s, i.audio.muted && (i.audio.muted = !1), 1 === s ? p.className = "demo-icon aplayer-icon-volume-up" : p.className = "demo-icon aplayer-icon-volume-down"
        }), p.addEventListener("click", function() {
          i.audio.muted ? (i.audio.muted = !1, p.className = 1 === i.audio.volume ? "demo-icon aplayer-icon-volume-up" : "demo-icon aplayer-icon-volume-down", i.updateBar("volume", i.audio.volume, "height")) : (i.audio.muted = !0, p.className = "demo-icon aplayer-icon-volume-off", i.updateBar("volume", 0, "height"))
        });
        var m = this.element.getElementsByClassName("aplayer-icon-loop")[0];
        m.addEventListener("click", function() {
          i.loop ? (m.classList.add("aplayer-noloop"), i.loop = !1, i.audio.loop = i.multiple ? !1 : i.loop) : (m.classList.remove("aplayer-noloop"), i.loop = !0, i.audio.loop = i.multiple ? !1 : i.loop)
      ...
      value: function(e) {
        ...
        if (this.music = this.multiple ? this.option.music[a] : this.option.music, this.music.pic && (this.element.getElementsByClassName("aplayer-pic")[0].style.backgroundImage = "url(" + encodeURI(this.music.pic) + ")"), this.element.getElementsByClassName("aplayer-title")[0].innerHTML = this.music.title, this.element.getElementsByClassName("aplayer-author")[0].innerHTML = " - " + this.music.author, this.multiple && (this.element.getElementsByClassName("aplayer-list-light")[0] && this.element.getElementsByClassName("aplayer-list-light")[0].classList.remove("aplayer-list-light"), this.element.getElementsByClassName("aplayer-list")[0].getElementsByTagName("li")[a].classList.add("aplayer-list-light")), this.audio && (this.pause(), this.audio.currentTime = 0), this.multiple && !this.audios[a] || -1 === this.playIndex ? (this.audio = document.createElement("audio"), this.audio.src = this.music.url, this.option.preload ? this.audio.preload = this.option.preload : this.audio.preload = this.isMobile ? "none" : "metadata", this.audio.addEventListener("durationchange", function() {
      ...
      value: function() {
        ...
        if (this.audio.paused) {
          if (this.button.classList.remove("aplayer-play"), this.button.classList.add("aplayer-pause"), this.button.innerHTML = "", setTimeout(function() {
            ...
            for (var a = 0; a < e.length; a++) this != e[a] && e[a].pause();
