...
得到的二进制数为 `10.0110011001100110011001100110011001100110011001100111B`，小数点往左移一位使得整数部分为 1，此时尾数部分为 53 位，进一舍零，于是得到最后的值是：`2^-2 * 1.0011001100110011001100110011001100110011001100110100`，这个值转化成真值，结果为：0.30000000000000004。

那么 `0.1 + 0.2 = 0.30000000000000004` 的推演到这里就结束了。
