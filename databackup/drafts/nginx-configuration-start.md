...
启动后，访问 <htttp://127.0.0.1:8888> ，如果 `/home/barretlee/test/` 下有 `index.html` 文件就会展示 `index.html` 的内容，否则返回 `404`。
...
首先 try_files，没找到就匹配静态资源，没找到就交给 Node 处理，否则就返回 4xx/5xx 的状态码。
